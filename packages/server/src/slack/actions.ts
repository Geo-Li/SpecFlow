...
export function registerActions(app: App): void {
  app.action("confirm_plan", async ({ ack, body, client }) => {
    await ack();
    const sessionId = getActionSessionId(body as BlockAction);
    ...
    try {
      ...
      if (result.success && result.prUrl) {
        ...
        await client.chat.postMessage({ channel: session.channelId!, thread_ts: session.threadTs!, text: "Execution failed. Check the admin dashboard for details." });
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : "Unknown error";
      console.error("Execution error:", err);
      updateSession(session.id, { status: "done", error: message });
      await client.chat.postMessage({ channel: session.channelId!, thread_ts: session.threadTs!, text: "Execution encountered an error. Check the admin dashboard for details." });
    }
  });
  ...
  app.action("edit_plan", async ({ ack, body, client }) => {
    ...
    if (session.userId !== userId) {
      await client.chat.postEphemeral({ channel: session.channelId!, user: userId, text: "Only the person who started this task can edit the plan." });
      return;
    }
  ...
  app.action("cancel_plan", async ({ ack, body, client }) => {
    ...
    if (session.userId !== userId) {
      await client.chat.postEphemeral({ channel: session.channelId!, user: userId, text: "Only the person who started this task can cancel." });
      return;
    }
